...
def linechart_maker(agency):
    df_agency = df[df['agency'] == agency].sort_values(by=['source','year'])
...
def barchart_maker(agency):
    ...
    barchart_df = barchart_df.query("dollar_change > 1000") #fix the "Treasury problem"
    ...
    ax = sns.barplot(data=barchart_df, x='Bureau', y='Value', hue='10-year Projections', palette={'22-31': '#84AE95', '25-34': '#004647'})

    for i in ax.containers:
        ax.bar_label(i, labels = [f"${x:,.0f}" for x in i.datavalues], weight="bold", fontsize = 12) #add labels to bars
    ...
    ax.set_xticklabels(textwrap.fill(x.get_text(), 30) for x in ax.get_xticklabels()) #x-axis labels on two lines if needed


    legend = ax.get_legend() # Get the current legend
    try:
        legend.set_title('')
...
if page == "Case Studies":
    ...
    agency_df = pd.read_excel('output/cbo_projection_changes.xlsx', sheet_name="agency increases").query("percent_change > 0 & Agency != 'Executive Office of the President'") #all agencies with an increase
    ...
    increase_by_program = pd.read_excel("output/cbo_projection_changes.xlsx", sheet_name="agency-bureau inc 2").query('percent_change > 0') #filter out decreases in spending
